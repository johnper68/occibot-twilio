...
function requireOnFinished () {
	if (hasRequiredOnFinished) return onFinishedExports;
	...
	onFinishedExports.isFinished = isFinished;
...
class TwilioWebHookServer extends node_events.EventEmitter {
    constructor(twilioPort) {
        ...
        this.handlerLocalMedia = (req, res) => {
            ...
            const mimeType = mimeTypes.lookup(decodeFile) || 'application/octet-stream';
...
class TwilioProvider extends bot.ProviderClass {
    constructor({ accountSid, authToken, vendorNumber, port = Number(process.env.PORT) || 3000, publicUrl = '', }) {
        super();
    ...
    async sendMedia(number, message, mediaInput) {
        ...
        const ecrypPath = bot.utils.encryptData(encodeURIComponent(mediaInput));
    ...
    async sendMessage(number, message, arg) {
        number = parseNumber(`${number}`);
        if (arg?.options?.buttons?.length)
            await this.sendButtons();
        if (arg?.options?.media)
            return this.sendMedia(number, message, arg?.options.media);
